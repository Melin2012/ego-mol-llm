...
**Unique-molecule (IK1) dedupe.** For free-form API evaluation we also release a **1-spectrum-per-InChIKey-first-block** subset of the 11 540 pack, stratified by official fold (prefer spectrum with more neighbor NIST hits, then lower HNSW index). This yields **1 780** jobs (train **1 652** / val **75** / test **53**) and is the recommended free-form set so spectrum-level accuracy does not overweight multi-replicate molecules. The full 11 540 pack remains available for spectrum-level ranker ablations.

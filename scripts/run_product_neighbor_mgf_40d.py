#!/usr/bin/env python3
"""
Product-style ranking for holdout-40d (neighbor + seed MGF first).

Design (user-agreed product path):
  - Primary structure evidence: GraphML neighbors + seed MGF (mass/adduct)
  - Optional: SIRIUS CSI, CFM neighbor structures (if precomputed)
...
Writes predictions_product_neighbor_mgf/ and PRODUCT_NEIGHBOR_MGF_RESULTS.json/csv.
Also mirrors predictions_grok_insession/ for multi-model folder naming.
"""
...
def main() -> int:
    pack = Path(r"C:\Users\AlexeyMelnik\Desktop\Blind_holdout40d_ego_msms")
    ...
    # Multi-model naming: same arm as in-session Grok product ranker
    out_grok = pack / "predictions_grok_insession"
    out_grok.mkdir(exist_ok=True)

    truth: dict[str, dict] = {}
    sealed = pack.parent / "Blind_holdout40d_ego_msms_SEALED_truth" / "truth_index.csv"
    if not sealed.is_file():
        print(f"WARNING: sealed truth not found at {sealed}; will write preds only")
    else:
        with sealed.open(encoding="utf-8-sig", newline="") as f:
    ...
    for row in csv.DictReader((pack / "sample_manifest.csv").open(encoding="utf-8", newline="")):
        ...
        d = {
            ...
            "model": "grok-insession-product-neighbor-mgf-v1",
            "blind": True,
        ...
        (out_dir / f"{sid}.json").write_text(payload, encoding="utf-8")
        (out_grok / f"{sid}.json").write_text(payload, encoding="utf-8")

        # score row (if sealed truth available)
        t = truth.get(sid)
        if t:
            ...
            print(
                f"[{len(rows_out)}/40] {sid} {pred.get('source')} "
                f"{(pred.get('smiles') or '')[:40]}"
            )

    n = len(rows_out)
    scored = [r for r in rows_out if "ik1" in r]
    if scored:
        ik = sum(1 for r in scored if r["ik1"])
        ex = sum(1 for r in scored if r["exact"])
        form_n = sum(1 for r in scored if r["formula_match"])
        t07 = sum(1 for r in scored if r["tani_ge_0.7"])
        t085 = sum(1 for r in scored if r["tani_ge_0.85"])
    else:
        ik = ex = form_n = t07 = t085 = 0
    n_pred = sum(1 for r in rows_out if r.get("pred_smiles"))
    summary = {
        ...
        "exact": ex,
        "exact_rate": ex / n if n else None,
        "formula_match": form_n,
        "formula_match_rate": form_n / n if n else None,
        "tani_ge_0.7": t07,
        "tani_ge_0.7_rate": t07 / n if n else None,
        "tani_ge_0.85": t085,
        "tani_ge_0.85_rate": t085 / n if n else None,
        "sources": dict(src_c),
    }
    (out_dir / "_batch_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    (out_grok / "_batch_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    ...
    print("\n=== PRODUCT NEIGHBOR+MGF SUMMARY (holdout40d) ===")
